...
test("visualizador abre em janela própria com tratamento de erros",async()=>{const [library,viewer]=await Promise.all([readFile(new URL("../app/drive-library.tsx",import.meta.url),"utf8"),readFile(new URL("../app/drive-document-page.tsx",import.meta.url),"utf8")]);assert.match(library,/window\.open\(`\/drive-document\?/);assert.match(library,/noopener,noreferrer/);assert.match(viewer,/response\.ok/);assert.match(viewer,/drive-reader-page/);assert.match(viewer,/role="alert"/)});
