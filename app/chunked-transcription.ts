...
export async function transcribeAudioInChunks(blob:Blob,onProgress:Progress,quality:TranscriptionQuality="accurate"){
 ...
 try{for(let sourceIndex=0;sourceIndex<chunks.length;sourceIndex++){onProgress(`Preparando áudio ${sourceIndex+1} de ${chunks.length}…`,Math.round((sourceIndex/chunks.length)*15));const decoded=await decode(chunks[sourceIndex]);const segmentSamples=16000*180,totalSegments=Math.max(1,Math.ceil(decoded.length/segmentSamples));for(let segmentIndex=0;segmentIndex<totalSegments;segmentIndex++){const start=segmentIndex*segmentSamples,end=Math.min(decoded.length,start+segmentSamples),segment=decoded.slice(start,end),part=segmentIndex+1;onProgress(`Preparando trecho ${part} de ${totalSegments}…`,15+Math.round((segmentIndex/totalSegments)*80));texts.push(await transcribeChunk(worker,segment,onProgress,part,totalSegments,quality));onProgress(`Trecho ${part} de ${totalSegments} concluído`,15+Math.round((part/totalSegments)*85));}}return texts.filter(Boolean).join("\n\n");}
