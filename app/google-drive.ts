...
const list = (items: string[], empty: string) =>
  items.length ? `<div class="doc-list">${items.map((item) => `<p>• ${escapeHtml(item)}</p>`).join("")}</div>` : `<p class="empty">${escapeHtml(empty)}</p>`;
