...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
 try{const user=await requireAccess(),{id}=await params,url=new URL(request.url),owner=user.role==="admin"&&url.searchParams.get("owner")?url.searchParams.get("owner")!.toLowerCase():user.email,row=await(await getRawDb()).prepare("SELECT audio_file_id FROM meetings WHERE email=? AND id=?").bind(owner,id).first<{audio_file_id:string}>();
