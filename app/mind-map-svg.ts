...
export function buildMindMapSvg(map: MindMapData) {
  ...
  branches.forEach((branch, branchIndex) => {
    ...
    nodes.push(node(topicX, topicY, topicWidth, 96, "#ffffff", branch.topic, "", "topic"));
    ...
    subtopics.forEach((subtopic, subIndex) => {
      ...
      nodes.push(node(leafX, leafY, leafWidth, 62, "#ffffff", subtopic, "", "subtopic"));
    });
    cursorY += blockHeight + blockGap;
  });

  const central = node(rootX, centerY, rootWidth, 112, "#ffffff", map.title, "", "center");
  const rootToggle = toggle(rootX + rootWidth / 2 + 42, centerY);
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xhtml="http://www.w3.org/1999/xhtml" viewBox="0 0 ${width} ${height}" role="img" aria-label="Mapa mental automático: ${xml(map.title)}"><rect width="100%" height="100%" fill="#ffffff"/><g class="mind-links">${paths.join("")}</g><g class="mind-nodes">${nodes.join("")}${central}</g><g class="mind-toggles">${toggles.join("")}${rootToggle}</g></svg>`;
}
...
function toggle(x: number, y: number) {
  return `<circle cx="${x}" cy="${y}" r="24" fill="#ffffff" stroke="#718077" stroke-width="2"/><text x="${x}" y="${y + 10}" text-anchor="middle" fill="#425047" font-family="Arial,sans-serif" font-size="32">‹</text>`;
}
