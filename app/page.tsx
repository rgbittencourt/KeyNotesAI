...
export default function Home() {
  ...
  const dashboardRecent = [...deviceRecordings.map(r=>({title:r.name,date:r.createdAt,duration:r.duration,tag:"Gravação",color:"#b98b4e",local:true})),...recent.map(r=>({...r,local:false}))].slice(0,5);
  if (!loggedIn) return (
      ...
      <section className="workspace">
        ...
        <div className="content">
          ...
          <section className="lower-grid">
            ...
            <div className="recent card"><div className="card-head"><div><p className="eyebrow">HISTÓRICO</p><h2>Reuniões recentes</h2></div><button onClick={() => setActive("Arquivos")}>Ver todas →</button></div>{dashboardRecent.map((r,i) => <button className="recent-row" key={`${r.title}-${i}`} onClick={() => r.local?setActive("Arquivos"):notify(`Abrindo ${r.title}`)}><span className="doc">{r.local?'▶':'☷'}</span><div><strong>{r.title}</strong><small>{r.date} · {r.duration}</small></div><em style={{'--tag': r.color} as React.CSSProperties}>{r.tag}</em><b>›</b></button>)}</div>
          </section></>}
        </div>
